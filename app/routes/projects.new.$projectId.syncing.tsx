...
import type { Route } from "./+types/projects.new.$projectId.syncing";
import { requireActiveAuth } from "~/lib/auth.server";
import { getProject, getProjectRepos } from "~/lib/projects.server";
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const repos = await getProjectRepos(params.projectId);

  if (repos.length === 0) {
    return redirect(`/projects/new/${params.projectId}/repos`);
  }

  const wasOnboarding = user.organization.onboardingCompleted;
  ...
  return {
    ...
    isPostOnboarding: wasOnboarding,
  };
}

export default function ProjectSyncing({ loaderData }: Route.ComponentProps) {
  const { projectId, projectName, repos, isPostOnboarding } = loaderData;
  ...
  const continueUrl = isPostOnboarding ? `/?project=${projectId}` : "/";
          ...
          <p className="text-neutral-500 dark:text-neutral-400">
            {allDone
              ? hasFailures
                ...
                : `All repositories for ${projectName} have been synced.`
